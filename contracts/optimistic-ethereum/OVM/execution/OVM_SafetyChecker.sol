...
contract OVM_SafetyChecker is iOVM_SafetyChecker {
     ...
     */
    function isBytecodeSafe(bytes memory _bytecode)
        external
        pure
        override
        returns (bool)
    {
        ...
        uint256[8] memory opcodeSkippableBytes =
            [
                uint256(
                    0x0001010101010101010101010000000001010101010101010101010101010000
                ),
                uint256(
                    0x0100000000000000000000000000000000000000010101010101000000010100
                ),
                uint256(
                    0x0000000000000000000000000000000001010101000000010101010100000000
                ),
                uint256(
                ...
                uint256(
                    0x0101010101000000000000000000000000000000000000000000000000000000
                ),
                uint256(
        ...
        uint256 opcodeHaltingMask =
            ~uint256(
                0x4008000000000000000000000000000000000000004000000000000000000001
            );
        // PUSH opcodes
        ...
        do {
            ...
            assembly {
                ...
                let indexInWord := byte(
                    0,
                    mload(add(opcodeSkippableBytes, byte(0, word)))
                )
                indexInWord := add(
                    indexInWord,
                    byte(
                        0,
                        mload(
                            add(opcodeSkippableBytes, byte(indexInWord, word))
                        )
                    )
                )
                indexInWord := add(
                ...
                if (opBit & opcodePushMask == 0) {
                    ...
                    do {
                        ...
                        if ((1 << opNum) & opcodePushMask == 0)
                            _pc += (opNum - 0x5f); // opNum-0x5f = PUSHed bytes (PUSH1 is 0x60)
                    ...
                    if ((firstOps >> 192) == 0x3350600060045af1) {
                        ...
                        firstOps ==
                        0x336000905af158600e01573d6000803e3d6000fd5b3d6001141558600a015760 &&
