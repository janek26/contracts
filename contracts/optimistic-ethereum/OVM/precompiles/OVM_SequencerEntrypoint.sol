...
contract OVM_SequencerEntrypoint {
    ...
    enum TransactionType { NATIVE_ETH_TRANSACTION, ETH_SIGNED_MESSAGE }
    ...
    fallback() external {
        TransactionType transactionType =
            _getTransactionType(Lib_BytesUtils.toUint8(msg.data, 0));

        bytes32 r =
            Lib_BytesUtils.toBytes32(Lib_BytesUtils.slice(msg.data, 1, 32));
        bytes32 s =
            Lib_BytesUtils.toBytes32(Lib_BytesUtils.slice(msg.data, 33, 32));
        ...
        bool isEthSignedMessage =
            transactionType == TransactionType.ETH_SIGNED_MESSAGE;
        ...
        if (Lib_SafeExecutionManagerWrapper.safeEXTCODESIZE(target) == 0) {
            ...
            bytes32 messageHash =
                Lib_ECDSAUtils.getMessageHash(encodedTx, isEthSignedMessage);
        ...
        Lib_SafeExecutionManagerWrapper.safeCALL(gasleft(), target, callbytes);
    }
     ...
     */
    function _getTransactionType(uint8 _transactionType)
        internal
        returns (TransactionType _txType)
    {
