...
contract Helper_TestRunner {
    ...
    function runSingleTestStep(TestStep memory _step) public {
        ...
        (bool success, bytes memory returndata) =
            address(msg.sender).call(_step.functionData);

        if (success != _step.expectedReturnStatus) {
            if (success == true) {
                console.log(
                    "ERROR: Expected function to revert, but function returned successfully"
                );
                ...
                console.log(
                    "ERROR: Expected function to return successfully, but function reverted"
                );
                console.log("Offending Step: %s", _step.functionName);
        ...
        if (keccak256(returndata) != keccak256(_step.expectedReturnData)) {
            if (success == true) {
                console.log(
                    "ERROR: Actual return data does not match expected return data"
                );
                console.log("Offending Step: %s", _step.functionName);
                ...
                if (_step.onlyValidateFlag) {
                    if (_expectedFlag != _flag) {
                        console.log(
                            "ERROR: Actual revert flag does not match expected revert flag data"
                        );
                        console.log("Offending Step: %s", _step.functionName);
                    ...
                    console.log(
                        "ERROR: Actual revert flag data does not match expected revert flag data"
                    );
                    console.log("Offending Step: %s", _step.functionName);
                    console.log("Expected Flag: %s", _expectedFlag);
                    console.log("Actual Flag: %s", _flag);
                    console.log(
                        "Expected Nuisance Gas Left: %s",
                        _expectedNuisanceGasLeft
                    );
                    console.log(
                        "Actual Nuisance Gas Left: %s",
                        _nuisanceGasLeft
                    );
                    console.log(
                        "Expected OVM Gas Refund: %s",
                        _expectedOvmGasRefund
                    );
    ...
    function runMultipleTestSteps(TestStep[] memory _steps) public {
    ...
    function _failStep() internal pure {
        revert("Test step failed.");
    }
}

contract Helper_TestRunner_CREATE is Helper_TestRunner {
    constructor(bytes memory _bytecode, TestStep[] memory _steps) {
        if (_steps.length > 0) {
